 ...
 *
 * `StepperInternalContextValue` is the wider value carried by the provider. It
 * adds compact-layout coordination for Stepper and Step but is not re-exported
 * from index.ts, so those implementation fields cannot widen the public hook.
 *
...
export const StepperContext = createContext<StepperInternalContextValue | null>(
  null,
);
StepperContext.displayName = 'StepperContext';

function useContextValue(hookName: string): StepperInternalContextValue {
...
export function useStepperInternalContext(): StepperInternalContextValue {
  return useContextValue('useStepperInternalContext');
}
...
export function useStepperContext(): StepperContextValue {
  return useContextValue('useStepperContext');
}
